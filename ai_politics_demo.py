...
def llm_call_async(prompt: str, model: str = "gpt-4o") -> str:
...
def run_llm_parallel(prompt_details: list) -> list:
    return [
...
def run_logos_workflow(speech_segment: str):
    ...
    with st.spinner("Orchestrator analyzing segment..."):
        orchestrator_response = llm_call_async(orchestrator_prompt, model="gpt-4o")
    ...
    with st.spinner("Workers verifying claims in parallel..."):
        worker_responses = run_llm_parallel(worker_prompt_details)
    ...
    with st.spinner("Formalizing claims into logical propositions..."):
        formalizer_responses = run_llm_parallel(formalizer_prompt_details)
    ...
    with st.spinner("Aggregator writing final report..."):
        final_report = llm_call_async(aggregator_prompt, model="gpt-4o")
...
def main():
    ...
    if st.button("🚀 Run Logos", disabled=not speech_segment):
        run_logos_workflow(speech_segment)
